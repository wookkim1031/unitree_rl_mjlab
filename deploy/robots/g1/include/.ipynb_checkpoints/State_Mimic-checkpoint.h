...
public:
    ...
    void load_data_from_npz(const std::string& motion_file)
    {
        ...
        auto body_lin_vel_w = npz_data["body_lin_vel_w"];  // [frame, body_id, 3]
        auto body_ang_vel_w = npz_data["body_ang_vel_w"];  // [frame, body_id, 3]
        ...
        anchor_lin_vels.clear();
        anchor_ang_vels.clear();
        ...
        for (size_t i = 0; i < num_frames_npz; i++)
        {
            ...
            // The anchor (torso_link) is a separate body in the buffer. Their code
            // reconstructs its orientation from root x waist joints instead, which
            // works for orientation but cannot give position or velocity — so read
            // the body directly.
            const size_t ai = ANCHOR_BODY_INDEX;
            const size_t pos_row = i * body_stride_pos  + ai * 3;
            const size_t quat_row = i * body_stride_quat + ai * 4;
            
            anchor_positions.push_back(
                Eigen::Vector3f::Map(body_pos_w.data<float>() + pos_row));
            anchor_quaternions.push_back(Eigen::Quaternionf(
                body_quat_w.data<float>()[quat_row + 0],   // w
                body_quat_w.data<float>()[quat_row + 1],   // x
                body_quat_w.data<float>()[quat_row + 2],   // y
                body_quat_w.data<float>()[quat_row + 3])); // z
            anchor_lin_vels.push_back(
                Eigen::Vector3f::Map(body_lin_vel_w.data<float>() + pos_row));
            anchor_ang_vels.push_back(
                Eigen::Vector3f::Map(body_ang_vel_w.data<float>() + pos_row));
    ...
    // Clamped like the Python at(): past the end the last frame repeats,
    // so lookahead never reads beyond the clip.
    int frame_at(int offset) const {
        return std::min(frame + offset, num_frames - 1);
    }
    Eigen::Vector3f    anchor_position(int off = 0) { return anchor_positions[frame_at(off)]; }
    Eigen::Quaternionf anchor_quaternion(int off = 0) { return anchor_quaternions[frame_at(off)]; }
    Eigen::Vector3f    anchor_lin_vel() { return anchor_lin_vels[frame]; }
    Eigen::Vector3f    anchor_ang_vel() { return anchor_ang_vels[frame]; }
    Eigen::VectorXf    joint_pos_at(int off) { return dof_positions[frame_at(off)]; }
    ...
    // torso_link in the 30-body deployment npz. In training the equivalent
    // index is 7, because mjlab's buffer holds only the 14 tracked bodies.
    static constexpr size_t ANCHOR_BODY_INDEX = 15;
    std::vector<Eigen::Vector3f>    anchor_positions;
    std::vector<Eigen::Quaternionf> anchor_quaternions;
    std::vector<Eigen::Vector3f>    anchor_lin_vels;
    std::vector<Eigen::Vector3f>    anchor_ang_vels;
};
